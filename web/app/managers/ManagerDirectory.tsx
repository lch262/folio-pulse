...
function ManagerCard({ manager, followed, onToggle }: { manager: ManagerProfile; followed: boolean; onToggle: () => void }) {
  return <article className={manager.status === "live" ? "manager-card live" : "manager-card"}>
    <a className="manager-card-link" href={`/manager/${manager.slug}`} aria-label={`打开 ${manager.nameZh} 主页`}>
      <div className="manager-card-top"><span className="manager-avatar">{manager.initials}</span><span className={`manager-status ${manager.status}`}>{manager.status === "live" ? "数据已接入" : "即将接入"}</span></div>
    ...
    <button className={followed ? "manager-follow followed" : "manager-follow"} type="button" aria-pressed={followed} onClick={onToggle}>{followed ? "✓ 已关注" : "+ 关注"}</button>
  </article>;
}
...
export default function ManagerDirectory() {
  ...
  return <>
    <div className="manager-toolbar">
      <label><span>搜索投资人或策略</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="例如：伯克希尔、宏观、集中持仓" /></label>
      <div className="manager-filter" role="group" aria-label="投资人筛选"><button className={filter === "ALL" ? "selected" : ""} type="button" onClick={() => setFilter("ALL")}>全部</button><button className={filter === "LIVE" ? "selected" : ""} type="button" onClick={() => setFilter("LIVE")}>已接入</button><button className={filter === "FOLLOWED" ? "selected" : ""} type="button" onClick={() => setFilter("FOLLOWED")}>我的关注 {followed.size}</button></div>
