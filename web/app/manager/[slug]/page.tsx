...
export default async function ManagerPage({ params }: PageProps) {
  ...
  const manager = getManager(slug);
  if (!manager) notFound();
  const isLive = manager.status === "live";
  const changes = portfolioSnapshot.positions.filter((item) => item.changeType !== "UNCHANGED").slice(0, 5);
  ...
  return <main className="route-page manager-profile-page">
    ...
    {isLive ? <section className="manager-live-content">
      <div className="manager-live-metrics"><article><span>最新报告期</span><strong>{portfolioSnapshot.reportDate}</strong><small>{portfolioSnapshot.filedAt} 提交</small></article><article><span>披露市值</span><strong>${portfolioSnapshot.totalValue.toFixed(2)}B</strong><small>SEC 13F-HR</small></article><article><span>持仓数量</span><strong>{portfolioSnapshot.positionCount}</strong><small>本季披露标的</small></article><article><span>发生变化</span><strong>{portfolioSnapshot.positionCount - portfolioSnapshot.changes.UNCHANGED}</strong><small>新建、增持、减持与清仓</small></article></div>
      <div className="manager-live-grid">
        <article className="manager-top-holdings"><div className="manager-panel-heading"><div><span className="section-kicker">Top holdings</span><h2>前五大持仓</h2></div><a href="/holdings">查看全部 →</a></div>{portfolioSnapshot.positions.slice(0, 5).map((item, index) => <a href={`/holding/${encodeURIComponent(item.ticker)}`} key={item.ticker}><span>0{index + 1}</span><strong>{item.ticker}</strong><small>{item.issuer}</small><em>{item.weight.toFixed(2)}%</em></a>)}</article>
        <article className="manager-latest-actions"><span className="section-kicker">Latest actions</span><h2>最新资金动作</h2><div>{changes.map((item) => <a href={`/holding/${encodeURIComponent(item.ticker)}`} key={item.ticker}><span className={`change-tag ${item.changeType.toLowerCase()}`}>{item.changeType === "ADDED" ? "增持" : item.changeType === "REDUCED" ? "减持" : item.changeType === "NEW" ? "新建仓" : "清仓"}</span><strong>{item.ticker}</strong><small>{item.shareChange > 0 ? "+" : ""}{compactShares(item.shareChange)}</small></a>)}</div><a className="manager-primary-link" href="/">打开完整仪表盘 →</a></article>
      </div>
